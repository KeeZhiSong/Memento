...
export async function PATCH(request: NextRequest) {
  const { reminderId, action, notificationKey, recurringTime } = await request.json();
  ...
  if (action === "update_recurring_time") {
    if (!recurringTime) {
      return NextResponse.json(
        { error: "recurringTime is required for update_recurring_time action" },
        { status: 400 },
      );
    }

    const updatedRecurring = updateRecurringReminderTime({
      reminderId,
      recurringTime,
    });
    if (!updatedRecurring) {
      return NextResponse.json(
        { error: "Recurring reminder not found" },
        { status: 404 },
      );
    }

    return NextResponse.json({ reminder: updatedRecurring });
  }
